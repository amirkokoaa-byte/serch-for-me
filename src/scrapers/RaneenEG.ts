...
export class RaneenEGScraper extends BaseScraper {
  readonly storeName = "Raneen";
  readonly baseUrl = "https://raneen.com/en";
  ...
  async searchTopProduct(query: string): Promise<StoreResult> {
    ...
    try {
      ...
      const items = $("div.product-item-info, li.product-item").toArray();
      if (items.length === 0) {
